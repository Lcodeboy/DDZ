...
public class GeoSearch {
	...
	static {
		...
		for (int angle = 1; angle <= GeoHash.MAX_PRECISION / 2; angle++) {
			BigDecimal bigDecimal = new BigDecimal(2).multiply(new BigDecimal(Math.PI)).multiply(EARTH_RADIUS)
	...
	public static List<GeoRange> range(double latitude, double longitude, double range) {
		...
		return getNineAroundCoordinate(latitude, longitude, desiredLength).stream().map(geoHash -> {
			...
			long min = longValue - 1 << (GeoHash.MAX_PRECISION - desiredLength);
			long max = (longValue + 2) << (GeoHash.MAX_PRECISION - desiredLength);
	...
	private static int getDesiredLength(double range) {
		int desiredLength = 0;
		Optional<BigDecimal> rangeKey = PRECISION_MAP.keySet().stream().sorted()
